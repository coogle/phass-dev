...
class GlassService implements \Zend\ServiceManager\ServiceLocatorAwareInterface, \Zend\EventManager\EventManagerAwareInterface
{
    ...
    public function setEventManager(\Zend\EventManager\EventManagerInterface $_eventManager) {
        ...
        $this->_eventManager->addIdentifiers('GoogleGlass\Service\GlassService');
        return $this;
    }
    ...
    /**
     * @param \GoogleGlass\Api\Client $_glassApiClient
     * @return self
     */
    ...
    public function getLocation($id = null)
    {
        switch(true) {
            case is_null($id):
                $itemId = "latest";
                break;
            case ($id instanceof \GoogleGlass\Entity\Subscription\Notification\Location):
                $itemId = $id->getItemId();
                break;
            case is_string($id):
                $itemId = $id;
    ...
    public function subscribe($collection, $operations = array(), $guid = null)
    {
        ...
        $subscription = $this->getServiceLocator()->get('GoogleGlass\Subscription');
    ...
    {
        $model = $this->getServiceLocator()->get('GoogleGlass\Model\CredentialsTable');
    ...
    public function isAuthenticated() {
        ...
        return $this->getServiceLocator()->get('GoogleGlass\OAuth2\Token')->isValid();
    }    
